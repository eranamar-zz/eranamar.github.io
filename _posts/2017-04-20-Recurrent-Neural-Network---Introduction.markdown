...
Recurrent Neural Networks (RNN) is a neural network architecture that lead to state of the art results in many tasks, such as machine translation, speech recognition, image to caption and many more. Because it is a rapidly evolving field, it is difficult to find all the information (both basic and advance material) in one place, so I decided to write a series of posts that will discuss Recurrent Neural Networks from their basic formulation to their most recent variations.
</div>
<div class="Indented">
This post, as the first in the series, will present the concept of recurrent neuron and its limitations. Next posts will discuss gated neurons like LSTM and GRU, the variants of dropout regularization specifically for RNN and some advance stuff that I found interesting (bi-directional RNN, Multiplicative LSTM and etc). 
</div>
<div class="Indented">
The next section is dedicated to review the notations and basic terminology that will be used during the entire series. I will assume that the reader is familiar with the basics of neural network (fully connected architecture, back-propagation algorithm, etc). If you are not familiar with one of those concepts, I strongly recommend review them online before going further. <a class="URL" href="https://www.academia.edu/25708860/A_Concise_Introduction_to_Machine_Learning_with_Artificial_Neural_Networks">Those notes </a> provides concise introduction to the field. There is also an excellent <a class="URL" href="https://www.youtube.com/watch?v=3BkFq55IEN8&amp;feature=youtu.be">video-lecture</a> (in Hebrew) by Prof. Shai Shalev-Shwartz from 2016. If you really want to dive deeper, you can find broad introduction in Chapter 6 in <i>Deep Learning</i> book (legally free electronic version <a class="URL" href="http://www.deeplearningbook.org/">here</a>).
</div>
<h1 class="Section">
...
<div class="Indented">
Neurons are organized within <i>layers</i>. In the very basic ANN architecture, called <i>fully connected (FC) neural network</i>, a layer is just a collection of neurons that will be computed in parallel (meaning that they are not connected to each other). For any layer <span class="MathJax_Preview"><script type="math/tex">
i
</script>
...
<div class="Indented">
The FC architecture is simple, yet yield a very powerful model. When wide enough, even a single hidden layer network can approximate to arbitrary precision almost any continuous function (with some mild assumptions). However, for some tasks it is not the optimal model, and different architectures are being used, instead of the FC one. 
</div>
<h1 class="Section">
<a class="toc" name="toc-Section-3">3</a> The Need for Context
</h1>
<div class="Unindented">
There are tasks that require <i>context</i> in order to make a prediction. For example, tasks that involved time-steps can consider the history until the current time step as a context (a.k.a time-series tasks). For concrete example, consider predicting the next word in a sentence. Each word have as a context all words preceding it in the sentence. Note that there are more ways to define the notion of a context, and it mostly depends on the task in hand. For didactic purposes we will focus on the example of next-word prediction. 
</div>
<div class="Indented">
...
</span>, that will be its state and be kept during consecutive invocations. As regard to the first computation of the neuron, we initialize the state variable to be zero so it will not affect the first computation.
</div>
<div class="Indented">
We adding an upper-script to all previous notations to denote the time step index starting from <span class="MathJax_Preview"><script type="math/tex">
1
</script>
</span> up to current time-step <span class="MathJax_Preview"><script type="math/tex">
t
</script>
...
</span>. Since the neurons need to pass information to themselves (i.e. their state from previous invocation) they become <i>recurrent neuron,</i> a.k.a <i>recurrent cell</i>. 
</div>
<div class="Indented">
Now the formulation of a neuron composed of two functions. First function updates the inner state based on <b>current input</b> and <b>previous state</b>, which is the transition from time <span class="MathJax_Preview"><script type="math/tex">
t-1
</script>
...
When we come to compare that to the FC formulation, we sees that actually the function <span class="MathJax_Preview"><script type="math/tex">
\mathbf{f}
</script>
</span> itself did not change, but now it accepts as input the state of the layer <span class="MathJax_Preview"><script type="math/tex">
\mathbf{h}
</script>
...
</span>, the networks update its state to <b><span class="MathJax_Preview"><script type="math/tex">
h^{\left(2\right)}
</script>
</span> </b>and produce <span class="MathJax_Preview"><script type="math/tex">
\hat{y}^{\left(2\right)}
</script>
</span> that is also being ignored. Lastly, we feed <span class="MathJax_Preview"><script type="math/tex">
x^{\left(3\right)}
</script>
</span> to the network, the inner state being updated to <span class="MathJax_Preview"><script type="math/tex">
h^{\left(3\right)}
</script>
</span> and another output is produced <span class="MathJax_Preview"><script type="math/tex">
\hat{y}^{\left(3\right)}
</script>
</span> which is now considered as the prediction of the network. Observed that in each invocation of the network, its output depends on both the current input and the previous state, therefore <span class="MathJax_Preview"><script type="math/tex">
\hat{y}^{\left(3\right)}
</script>
...
Parameter sharing have a very powerful effect on the learnability of the networks both because it reduce the number of parameters that needed to be optimize and it make the network robust to small changes in the context (recall the example about the order of features in the input). In addition, the recurrent layer practically have <i>dynamic</i> window size. Going back to our next-word prediction network, we can continue to apply the network on consecutive inputs, teaching it dependencies between words that are farer than 3 time steps. 
</div>
<div class="Indented">
...
<div class="Unindented">
Unfortunately, RNN are not perfect and learning to remember is not enough, sometimes the networks needs to be able to <i>forget</i>, for instance, when predicting next-word in a sentence the network needs to reset its state once it reach the end of the sentence. 
</div>
<div class="Indented">
Furthermore, parameter sharing has a serious drawback. Recall that applying the network for multiple time steps leads to composition of the same <span class="MathJax_Preview"><script type="math/tex">
\mathbf{s}\in\mathcal{S}
</script>
</span> on itself <span class="MathJax_Preview"><script type="math/tex">
t
</script>
...
\mathbf{h}^{\left(t\right)}=\mathbf{s}\left(\mathbf{x}^{\left(t\right)},\mathbf{s}\left(\mathbf{x}^{\left(t-1\right)},\mathbf{s}\left(\mathbf{x}^{\left(t-2\right)},...\right)\right)\right)
 ...
 in order to analyze that more easily, we make the following simplifications: assume that the activation function inside <span class="MathJax_Preview"><script type="math/tex">
\mathbf{s}
</script>
...
</span> is given to us and it is not all zeros (that is, it encodes some information about the past). We now analyze how the recurrent connection behave over time.
</div>
<div class="Indented">
...
</span>. Why it is bad? it easier to see the problem when considering the scalar case. If <span class="MathJax_Preview"><script type="math/tex">
U
</script>
...
</span> to vanish, and the large entries to explode. Therefore, some coordinates of <span class="MathJax_Preview"><script type="math/tex">
\mathbf{h}^{\left(0\right)}
</script>
</span> will eventually become zero while the others increase to infinity, resulting in a distorted state vector that will impair the learnability of the network. That is describe much more formally and rigorously <a class="URL" href="https://arxiv.org/abs/1211.5063">here</a>.
</div>
<div class="Indented">
